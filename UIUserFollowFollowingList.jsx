...
const EXTRA_PAGES = [
  ...
  { id: 'stats', label: 'Stats', icon: User, color: 'text-slate-500', bg: 'bg-slate-500' },
  { id: 'user-profile', label: 'User Profile', icon: User, color: 'text-slate-500', bg: 'bg-slate-500' },
];
...
// Added isPrivate flag to demonstrate privacy logic
const USERS_LIST = [
  { id: 1, name: 'Sarah Wilson', handle: '@sarah_w', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=150&q=80', isFollowing: true, isPrivate: true, bio: 'Nature lover 🌿 | Photography 📷' },
  { id: 2, name: 'Mike Chen', handle: '@mike_c', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80', isFollowing: false, isPrivate: true, bio: 'Tech enthusiast. Private account.' },
  { id: 3, name: 'Alex Morgan', handle: '@alex_m', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80', isFollowing: true, isPrivate: false, bio: 'Just living life one day at a time.' },
  { id: 4, name: 'Emily Rose', handle: '@emily_r', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?auto=format&fit=crop&w=150&q=80', isFollowing: false, isPrivate: false, bio: 'Artist & Designer 🎨' },
  { id: 5, name: 'David Kim', handle: '@david_k', avatar: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=150&q=80', isFollowing: true, isPrivate: true, bio: 'Musician 🎸' },
  { id: 6, name: 'Jessica Lee', handle: '@jess_lee', avatar: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=150&q=80', isFollowing: false, isPrivate: true, bio: 'Private life.' },
];
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 animate-fade-in">
      ...
      <div className="flex border-b border-slate-100 dark:border-slate-800 px-4 mt-2">
        <TabItem id="followers" label={`${MY_PROFILE_DATA.followers} Followers`} />
        <TabItem id="following" label={`${MY_PROFILE_DATA.following} Following`} />
            ...
            <div key={user.id} onClick={() => onUserClick(user.id)} className="flex items-center justify-between p-3 mb-1 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-xl transition-colors cursor-pointer group">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {activeTab === 'suggestions' && <p className="text-[10px] text-slate-400 mt-0.5">Followed by mike_c + 2 more</p>}
                </div>
              </div>
              <button 
                onClick={(e) => e.stopPropagation()} // Prevent row click when clicking button
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
                user.isFollowing && activeTab !== 'suggestions'
                ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700'
                : 'bg-blue-500 text-white hover:bg-blue-600 shadow-md shadow-blue-500/20'
              }`}>
                {user.isFollowing && activeTab !== 'suggestions' ? 'Following' : 'Follow'}
              </button>
            </div>
          ))}
...
const ProfileView = ({ onOpenStats, userId }) => {
  ...
  if (userId && userId !== 'me') {
    const foundUser = USERS_LIST.find(u => u.id === userId);
    if (foundUser) {
      // Create a mock full profile object from the partial user list data
      profileData = {
        ...foundUser,
        isCurrentUser: false,
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto pb-20 animate-fade-in">
      ...
      {/* 2. Navigation Tabs OR Private State */}
      {isPrivateRestricted ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center border-t border-slate-100 dark:border-slate-800">
...
const ContentArea = ({ activeTab, onPushView, statsTab, viewParams }) => {
  // If active tab is Profile, render the new component
  ...
  // Handle generic User Profile View
  if (activeTab === 'user-profile') {
    const userId = viewParams?.[activeTab]?.userId;
    return (
      <div className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth relative h-full">
  ...
  // If active tab is Stats, render StatsView
  if (activeTab === 'stats') {
    return (
      <div className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth relative h-full">
        <StatsView 
          initialTab={viewParams?.initialTab || statsTab} 
